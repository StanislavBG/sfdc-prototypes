...
function SalesforceLogo() {
  return (
    <svg viewBox="0 0 48 32" className="h-6 w-9" fill="white">
...
export default function Header({
  ...
  return (
    <>
      ...
      {/* Main header */}
      <header className="sf-header flex items-center px-3 gap-2">
        {/* Left: Logo + App Launcher + App Name */}
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <span className="text-white text-sm font-semibold ml-1 whitespace-nowrap">
            {currentAppData?.name || 'Data Cloud'}
          </span>
        </div>

        {/* Center: Navigation Tabs */}
        <nav className="flex items-center h-full overflow-x-auto flex-1 ml-2 scrollbar-hide">
          {navTabs.map((tab) => (
            <button
              key={tab.label}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {/* Search scope dropdown + search */}
          <div className="flex items-center">
            <GlobalSearch onSelectResult={onSelectSearchResult} />
          </div>

          {/* Right icons - matching screenshot order */}
          ...
          {/* Edit pencil icon (shown at far right in screenshot) */}
